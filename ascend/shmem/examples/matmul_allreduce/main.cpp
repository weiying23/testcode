...
{
    // 设置FFTS（Fast Flag Task Sync）配置地址，用于核间快速同步通信
    ...
    // CopyDirect::Put表示使用shmem_put方式，从本地主动推送数据到远端rank
    ...
    // AllGather将各rank的部分数据收集后广播到所有rank
    ...
    // AllGather操作: 各rank的部分数据被收集并广播到所有rank，每个rank都获得完整数据
    ...
    // 1. 从各rank的对称内存读取对应的规约结果部分(使用shmem_get)
    // 2. 将收集到的数据组装成完整的规约结果矩阵
    // 3. 将完整结果写入到本地对称内存，供所有rank访问
    // 参数: EpilogueAllGatherDispatch-分发策略, RemoteSrcType/RemoteDstType-数据类型,
    //       CommCoreSplit-核分裂配置, CommBlockShape-通信块形状,
    //       EpilogueAllGatherTileShape-tile形状, TileRemoteCopy-远程拷贝实现, TileScheduler-tile调度
    ...
    // COMM_INTERVAL: 通信间隔，定义每隔3次矩阵乘法tile计算后执行一次通信操作
    // 该参数控制计算和通信的交替频率，平衡计算效率和通信带宽利用
    constexpr uint32_t COMM_INTERVAL = 3;
    // MatmulAllReduceKernel: Matmul+AllReduce组合Kernel类
    // 实现矩阵乘法与AllReduce通信的融合执行，利用shmem实现高效的分布式计算
    // 执行流程:
    // 1. 执行部分矩阵乘法计算(本地A×B得到部分C结果)
    // 2. 使用shmem ReduceScatter将部分C结果规约并分发到各rank
    // 3. 使用shmem AllGather收集规约后的完整结果
    // 参数: BlockMmad-矩阵乘法块, BlockEpilogueReduceScatter-ReduceScatter通信块,
    //       BlockEpilogueAllGather-AllGather通信块, BlockMmadScheduler-计算调度器,
    //       BlockEpilogueScheduler-通信调度器, WORKSPACE_STAGES-工作空间阶段数
    ...
    // 存储AllGather操作的具体配置参数，如数据偏移、数据组装顺序等
    typename BlockEpilogueAllGather::Params allGatherParams{};

    // MatmulAllReduceKernel::Params: Matmul+AllReduce Kernel参数结构体
    ...
    typename MatmulAllReduceKernel::Params params{
        ...
        COMM_INTERVAL,              // 通信间隔: 每隔多少次计算后执行通信
        gmA, layoutA,               // 矩阵A: 全局内存地址和数据布局
        gmB, layoutB,               // 矩阵B: 全局内存地址和数据布局
        gmD, layoutD,               // 矩阵D(输出): 全局内存地址和数据布局
...
int main(int argc, char **argv)
{
    ...
    // shmem_set_conf_store_tls(): 禁用TLS(Thread Local Storage)存储配置方式，设置为false后使用自定义配置方式初始化shmem
    ...
    // 该内存用于存储AllReduce操作中的中间通信数据，实现跨rank的数据聚合和广播
    ...
    for (int i = 0; i < 1; i++) {
        ...
        // kernel内部会使用此地址进行AllGather和ReduceScatter操作的核间同步
