...
int test_aclshmem_team_all_gather(int pe_id, int n_pes, uint64_t local_mem_size)
{
    ...
    // - stream: ACL流
    // 执行效果:
    // - 阻塞直到handle指定的AllGather操作完成
    // - 用于异步操作的同步等待
    // - 与shmem_quiet相比，可以等待特定操作而非所有操作
    // handle_wait优势：
    // - 更细粒度的同步控制，只等待特定操作
    // - 避免等待所有pending操作，提高效率
    // - 可以在AllGather完成后立即执行后续操作
    aclshmemx_handle_wait(handle, stream);

    // ========== 数据拷贝操作 ==========
    // copy_demo: 将AllGather后的数据从ptr拷贝到ptr_A
    // 此操作在handle_wait之后执行，确保数据已完全gather
    copy_demo(1, stream, ptr, ptr_A, n_pes * trans_size * sizeof(int32_t));
    status = aclrtSynchronizeStream(stream);

    // ========== 结果校验 ==========
    // 校验ptr_A中的AllGather结果是否正确
    // 由于使用了handle_wait，数据应该完全正确
    if (pe_id <= n_pes) {
        int32_t *y_host;
        ...
        // aclrtMemcpy: 将Device端的AllGather结果拷贝到Host端进行校验
        status = aclrtMemcpy(y_host, input_size, ptr_A, input_size, ACL_MEMCPY_DEVICE_TO_HOST);
        std::cout << "Relative pe " << pe_id << " AllGather result in ptr_A after handle_wait:" << std::endl;

        // 校验每个PE的数据：期望值 = num10 + pe_index
        // handle_wait确保AllGather完成，数据应该完全正确
    ...
    // 2. 所有PE应同时释放对称内存
    // 3. 释放前确保RDMA操作已完成（handle_wait已调用）
    aclshmem_free(ptr);
