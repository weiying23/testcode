    ...
    uint32_t m, uint32_t n, uint32_t k)
{
    ...
    // 行数固定为64，列数动态计算(UINT_MAX / 2)以适应不同矩阵宽度
    ...
    // 定义shmem AllGather通信操作中每个tile处理的数据维度（32行×256列）
    ...
    // AllGatherMatmulKernel: AllGather+Matmul+Padding组合Kernel类
    // 实现AllGather通信与矩阵乘法计算的融合执行，并支持矩阵B的Padding处理
    ...
    // 参数: GlobalPaddingB-矩阵B的全局Padding策略, BlockMmad-矩阵乘法块,
    ...
    // 存储AllGather操作的具体配置参数，初始化为空
    typename BlockEpilogueAllGather::Params allGatherParams{};

    // Prepare params
    // AllGatherMatmulKernel::Params: AllGather+Matmul+Padding Kernel参数结构体
    ...
    typename AllGatherMatmulKernel::Params params{
        ...
        workSpace, layoutWB,        // 工作空间: 用于Padding操作的临时内存和布局
