...
int main(int argc, char **argv)
{
    ...
    // 包含以下关键字段：
    // - my_pe: 当前PE编号（进程ID），用于标识本进程在通信组中的位置
    //   范围[0, n_pes-1]，在MoE场景中对应rank_id
    // - n_pes: 总PE数量（进程总数），对应EP（Expert Parallelism）大小
    // - ip_port: rendezvous地址（TCP socket地址），格式如"tcp://127.0.0.1:8998"
    //   PE 0作为server监听端口，其他PE作为client连接
    // - local_mem_size: 对称内存大小（字节），用于存储MoE中间结果
    // - option_attr: 可选属性
    ...
    // 参数详解:
    // - rank_id: 当前PE编号（MoE中的rank，对应进程ID）
    // - n_ranks: 总PE数量（对应EP大小，即专家并行度）
    ...
    // MoE场景中的作用：
    // 1. 建立所有rank之间的通信通道
    // 2. 分配对称内存堆，用于存储expert dispatch中间结果
    // 3. 初始化通信引擎，支持expert weight在不同rank间传输
    // 4. 设置rank_id和n_ranks信息
    ...
    // 参数详解:
    // - aclshmem_size: 对称内存大小（约504MB * 2字节 = ~1GB）
    //   用于存储expert dispatch过程中的中间数据
    // 返回值: 对称内存指针（GVA格式）
    // GVA = Global Virtual Address，全局虚拟地址
    // MoE场景中的用途：
    // - 存储allgather后的expanded tokens
    // - 存储reduce_scatter前的部分矩阵乘法结果
    // - 存储expert weight传输过程中的缓冲数据
    // 对称内存核心特点：
    // 1. 所有PE在同一虚拟地址上拥有相同大小的内存块
    // 2. PE i可以直接通过GVA地址访问PE j的数据
    // 3. 用于存放通信数据和同步标志
    // 注意：
    // - 对称内存必须通过aclshmem_free释放，不能使用aclrtFree
