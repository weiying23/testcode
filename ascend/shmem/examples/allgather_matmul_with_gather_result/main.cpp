...
)
{
    ...
    // BlockSchedulerCopyGatherA: 收集矩阵A数据的块调度器，专门用于将AllGather收集的矩阵A数据拷贝到输出位置
    // 该调度器协调矩阵A的收集结果拷贝操作，使其与计算操作并行执行
    ...
    // 在AllGather操作中，各rank将自己的矩阵A推送到对称内存，RemoteDstType是GatherAType
    ...
    // CORE_SPLIT_ROWS: 核分裂行数，定义参与AllGather通信操作的AIV核心数量为16个
    ...
    // GATHER_BLOCK_ROWS: 收集拷贝块行数，定义拷贝收集结果时每次处理的数据行数为48行
    // 该参数影响矩阵A收集结果拷贝操作的粒度
    constexpr uint32_t GATHER_BLOCK_ROWS = 48;
    // GATHER_BLOCK_COLUMNS_DIVISOR: 收集拷贝块列数除数，用于计算收集块的列数上限
    constexpr uint32_t GATHER_BLOCK_COLUMNS_DIVISOR = 2;
    // GATHER_TILE_ROWS: 收集拷贝Tile行数，定义每个tile处理48行数据
    constexpr uint32_t GATHER_TILE_ROWS = 48;
    // GATHER_TILE_COLUMNS: 收集拷贝Tile列数，定义每个tile处理1024列数据
    constexpr uint32_t GATHER_TILE_COLUMNS = 1024;
    // CopyGatherABlockShape: 收集矩阵A的拷贝块形状类型
    // 定义将AllGather收集的矩阵A数据从对称内存拷贝到输出位置的数据块维度
    using CopyGatherABlockShape = Catlass::MatrixShape<GATHER_BLOCK_ROWS, UINT_MAX / GATHER_BLOCK_COLUMNS_DIVISOR>;
    // CopyGatherATileShape: 收集矩阵A的Tile形状类型
    using CopyGatherATileShape = Catlass::MatrixShape<GATHER_TILE_ROWS, GATHER_TILE_COLUMNS>;
    // CopyGatherADispatchPolicy: 收集矩阵A的本地拷贝分发策略
    // 参数: UB_STAGES-缓冲区阶段数，用于流水线化本地数据拷贝操作
    // EpilogueAtlasA2CommLocalCopy专门用于本地内存拷贝，不涉及跨rank通信
    using CopyGatherADispatchPolicy = CommEpilogue::EpilogueAtlasA2CommLocalCopy<UB_STAGES>;
    // BlockEpilogueCopyGatherA: 收集矩阵A的Epilogue块类
    // 封装将AllGather收集的完整矩阵A数据从对称内存拷贝到gmGatherA输出位置的实现
    // 参数: CopyGatherADispatchPolicy-本地拷贝策略, AType/GatherAType-数据类型,
    //       CopyGatherABlockShape-拷贝块形状, CopyGatherATileShape-tile形状, TileScheduler-tile调度
    ...
    // AllGatherMatmulWithGatherResultKernel: AllGather+Matmul+GatherResult组合Kernel类
    // 实现AllGather通信与矩阵乘法计算的融合执行，并将收集的矩阵A输出保存
    // 执行流程:
    // 1. 各rank并行执行本地矩阵乘法计算的部分工作(A_local × B_local)
    // 2. 同时执行AllGather通信，将各rank的矩阵A收集到对称内存
    // 3. 将收集到的完整矩阵A集合拷贝到gmGatherA输出位置
    // 4. 使用收集到的完整矩阵A集合继续执行矩阵乘法
    // 5. 最终每个rank都得到完整的矩阵乘法结果 C = A_all × B_local 和收集的矩阵A数据
    // 参数: BlockMmad-矩阵乘法块, BlockEpilogueAllGather-AllGather通信块,
    //       BlockEpilogueCopyGatherA-收集结果拷贝块, BlockScheduler-计算调度器(AllGather专用),
    //       BlockCommScheduler-通信调度器, BlockCopyGatherAScheduler-收集拷贝调度器,
    ...
    // copyGatherAParams: 收集矩阵A拷贝参数结构体
    // 存储将收集结果拷贝到输出位置的配置参数
    typename BlockEpilogueCopyGatherA::Params copyGatherAParams{};

    // Prepare params
    // AllGatherMatmulWithGatherResultKernel::Params: AllGather+Matmul+GatherResult Kernel参数结构体
    ...
    typename AllGatherMatmulWithGatherResultKernel::Params params{
        ...
        copyGatherAParams,          // 收集矩阵A拷贝参数: 将收集结果拷贝到输出的配置
        gmGatherA, layoutGatherA,   // 收集的矩阵A输出: 全局内存地址和数据布局（收集后的完整矩阵A）
        gmC, layoutC,               // 矩阵C(输出): 全局内存地址和数据布局（收集后的完整计算结果）
