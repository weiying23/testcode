...
)
{
    ...
    // 参数: 7-调度组大小，定义每个调度组包含7个block
    // 参数: 1-调度方向，定义block的调度顺序方向
    ...
    // CommBlockShape: 通信块形状类型，定义shmem通信操作的基本数据块维度（64行×256列）
    ...
    // 定义shmem ReduceScatter通信操作中每个tile处理的数据维度（32行×256列）
    ...
    // MatmulReduceScatterKernel: Matmul+ReduceScatter+Padding组合Kernel类
    // 实现矩阵乘法与ReduceScatter通信的融合执行，并支持矩阵A和B的Padding处理
    // Padding用于处理矩阵维度不满足tile大小要求的情况，通过填充零值使维度对齐
    // 执行流程:
    // 1. 执行矩阵乘法计算(本地A×B得到完整C结果)
    // 2. 使用shmem ReduceScatter将C结果按行分块规约并分发到各rank
    // 3. 每个rank得到自己对应部分的规约结果 C_local = sum(C_i[block])
    // 参数: GlobalPaddingA-矩阵A的全局Padding策略, GlobalPaddingB-矩阵B的全局Padding策略,
    //       BlockMmad-矩阵乘法块, BlockEpilogueReduceScatter-ReduceScatter通信块,
    ...
    // MatmulReduceScatterKernel::Params: Matmul+ReduceScatter+Padding Kernel参数结构体
    ...
    typename MatmulReduceScatterKernel::Params params{
        ...
        gmWA, layoutWA,             // 矩阵A工作空间: 用于Padding操作的临时内存和布局
        gmWB, layoutWB,             // 矩阵B工作空间: 用于Padding操作的临时内存和布局
