...
/**
 * @file highlevel_mem_mte_test.cpp
 * @brief Combined unit tests for high-level RMA interfaces under MTE engine
 *
...
// Device kernel functions (defined in highlevel_mem_mte_test_kernel.cpp)
extern void test_mte_getmem(uint32_t block_dim, void* stream, uint8_t* gva, uint64_t config);
extern void test_mte_putmem(uint32_t block_dim, void* stream, uint8_t* gva, uint64_t config);
extern void test_mte_getmem_nbi(uint32_t block_dim, void* stream, uint8_t* gva, uint64_t config);
extern void test_mte_putmem_nbi(uint32_t block_dim, void* stream, uint8_t* gva, uint64_t config);
 ...
 */
static void test_mte_put_get_mem_func(aclrtStream stream, uint8_t* gva, uint32_t pe_id, uint32_t pe_size)
{
    ...
    test_mte_putmem(block_dim, stream, gva, util_get_ffts_config());
    ...
    ASSERT_EQ(aclrtMemcpy(gva, totalSize, inHost, totalSize, ACL_MEMCPY_HOST_TO_DEVICE), 0);
    aclshmemi_control_barrier_all();
    test_mte_getmem(block_dim, stream, gva, util_get_ffts_config());
    ASSERT_EQ(aclrtSynchronizeStream(stream), 0);
    aclshmemi_control_barrier_all();
    ASSERT_EQ(aclrtMemcpy(outHost, totalSize, gva, totalSize, ACL_MEMCPY_DEVICE_TO_HOST), 0);
    for (uint32_t i = 0; i < pe_size; i++) {
...
void test_aclshmem_mte_put_get_mem(int pe_id, int n_pes, uint64_t local_mem_size)
{
    ...
    test_mte_put_get_mem_func(stream, (uint8_t*)g_state.heap_base, pe_id, n_pes);
    std::cout << "[TEST] putmem+getmem (MTE) finished, pe_id: " << pe_id << std::endl;
    test_finalize(stream, device_id);
}

/**
 * Combined unit test for aclshmem_putmem + aclshmem_getmem under MTE engine
 */
TEST(TestHighlevelMemApi, TestShmemPutGetMemBlockingMTE)
{
    const int process_count = test_gnpu_num;
    uint64_t local_mem_size = 1024UL * 1024UL * 1024;
    test_mutil_task(test_aclshmem_mte_put_get_mem, local_mem_size, process_count);
}
 ...
 */
static void test_mte_put_get_mem_nbi_func(aclrtStream stream, uint8_t* gva, uint32_t pe_id, uint32_t pe_size)
{
    size_t messageSize = 64;
    uint32_t peOffset = 10;
    uint32_t* inHost;
    ...
    ASSERT_EQ(aclrtMemcpy(gva, totalSize, inHost, totalSize, ACL_MEMCPY_HOST_TO_DEVICE), 0);
    aclshmemi_control_barrier_all();
    test_mte_putmem_nbi(block_dim, stream, gva, util_get_ffts_config());
    ASSERT_EQ(aclrtSynchronizeStream(stream), 0);
    aclshmemi_control_barrier_all();
    ASSERT_EQ(aclrtMemcpy(outHost, totalSize, gva, totalSize, ACL_MEMCPY_DEVICE_TO_HOST), 0);
    for (uint32_t i = 0; i < pe_size; i++) {
    ...
    ASSERT_EQ(aclrtMemcpy(gva, totalSize, inHost, totalSize, ACL_MEMCPY_HOST_TO_DEVICE), 0);
    aclshmemi_control_barrier_all();
    test_mte_getmem_nbi(block_dim, stream, gva, util_get_ffts_config());
    ASSERT_EQ(aclrtSynchronizeStream(stream), 0);
    aclshmemi_control_barrier_all();
    ASSERT_EQ(aclrtMemcpy(outHost, totalSize, gva, totalSize, ACL_MEMCPY_DEVICE_TO_HOST), 0);
    for (uint32_t i = 0; i < pe_size; i++) {
...
void test_aclshmem_mte_put_get_mem_nbi(int pe_id, int n_pes, uint64_t local_mem_size)
{
    int32_t device_id = pe_id % test_gnpu_num + test_first_npu;
    aclrtStream stream;
    test_init(pe_id, n_pes, local_mem_size, &stream);
    ...
    test_mte_put_get_mem_nbi_func(stream, (uint8_t*)g_state.heap_base, pe_id, n_pes);
    std::cout << "[TEST] putmem_nbi+getmem_nbi (MTE) finished, pe_id: " << pe_id << std::endl;
    test_finalize(stream, device_id);
}

/**
 * Combined unit test for aclshmem_putmem_nbi + aclshmem_getmem_nbi under MTE engine
 */
TEST(TestHighlevelMemApi, TestShmemPutGetMemNbiMTE)
{
    const int process_count = test_gnpu_num;
    uint64_t local_mem_size = 1024UL * 1024UL * 1024;
    test_mutil_task(test_aclshmem_mte_put_get_mem_nbi, local_mem_size, process_count);
}
...
#define DECLARE_PUT_GET_WRAPPER(NAME, TYPE)                                                             \
    extern void test_mte_##NAME##_put(uint32_t block_dim, void* stream, uint8_t* gva, uint64_t config); \
    extern void test_mte_##NAME##_get(uint32_t block_dim, void* stream, uint8_t* gva, uint64_t config)
...
#define DEFINE_PUT_GET_TEST(NAME, TYPE)                                                                            \
    static void test_mte_##NAME##_put_get_func(aclrtStream stream, uint8_t* gva, uint32_t pe_id, uint32_t pe_size) \
    {                                                                                                              \
        size_t messageSize = 64;                                                                                   \
        uint32_t peOffset = 10;                                                                                    \
        TYPE* inHost;                                                                                              \
        ...
        ASSERT_EQ(aclrtMemcpy(gva, totalSize, inHost, totalSize, ACL_MEMCPY_HOST_TO_DEVICE), 0);                   \
        aclshmemi_control_barrier_all();                                                                           \
        test_mte_##NAME##_put(block_dim, stream, gva, util_get_ffts_config());                                     \
        ASSERT_EQ(aclrtSynchronizeStream(stream), 0);                                                              \
        aclshmemi_control_barrier_all();                                                                           \
        ASSERT_EQ(aclrtMemcpy(outHost, totalSize, gva, totalSize, ACL_MEMCPY_DEVICE_TO_HOST), 0);                  \
        for (uint32_t i = 0; i < pe_size; i++) {                                                                   \
        ...
        ASSERT_EQ(aclrtMemcpy(gva, totalSize, inHost, totalSize, ACL_MEMCPY_HOST_TO_DEVICE), 0);                   \
        aclshmemi_control_barrier_all();                                                                           \
        test_mte_##NAME##_get(block_dim, stream, gva, util_get_ffts_config());                                     \
        ASSERT_EQ(aclrtSynchronizeStream(stream), 0);                                                              \
        aclshmemi_control_barrier_all();                                                                           \
        ASSERT_EQ(aclrtMemcpy(outHost, totalSize, gva, totalSize, ACL_MEMCPY_DEVICE_TO_HOST), 0);                  \
        for (uint32_t i = 0; i < pe_size; i++) {                                                                   \
...
#define DEFINE_PUT_GET_ENTRY(NAME, TYPE)                                                                 \
    void test_aclshmem_mte_##NAME##_put_get(int pe_id, int n_pes, uint64_t local_mem_size)               \
    {                                                                                                    \
        int32_t device_id = pe_id % test_gnpu_num + test_first_npu;                                      \
        aclrtStream stream;                                                                              \
        test_init(pe_id, n_pes, local_mem_size, &stream);                                                \
        ASSERT_NE(stream, nullptr);                                                                      \
                                                                                                         \
        test_mte_##NAME##_put_get_func(stream, (uint8_t*)g_state.heap_base, pe_id, n_pes);               \
        std::cout << "[TEST] " #NAME "_put+" #NAME "_get (MTE) finished, pe_id: " << pe_id << std::endl; \
        test_finalize(stream, device_id);                                                                \
    }                                                                                                    \
                                                                                                         \
    TEST(TestTypedPutGetMTE, TestShmem##NAME##PutGetBlockingMTE)                                         \
    {                                                                                                    \
        const int processCount = test_gnpu_num;                                                          \
        uint64_t local_mem_size = 1024UL * 1024UL * 1024;                                                \
        test_mutil_task(test_aclshmem_mte_##NAME##_put_get, local_mem_size, processCount);               \
    }
...
#define DECLARE_PUT_GET_NBI_WRAPPER(NAME, TYPE)                                                             \
    extern void test_mte_##NAME##_put_nbi(uint32_t block_dim, void* stream, uint8_t* gva, uint64_t config); \
    extern void test_mte_##NAME##_get_nbi(uint32_t block_dim, void* stream, uint8_t* gva, uint64_t config)
...
#define DEFINE_PUT_GET_NBI_TEST(NAME, TYPE)                                                                            \
    static void test_mte_##NAME##_put_get_nbi_func(aclrtStream stream, uint8_t* gva, uint32_t pe_id, uint32_t pe_size) \
    {                                                                                                                  \
        size_t messageSize = 64;                                                                                       \
        uint32_t peOffset = 10;                                                                                        \
        TYPE* inHost;                                                                                                  \
        ...
        ASSERT_EQ(aclrtMemcpy(gva, totalSize, inHost, totalSize, ACL_MEMCPY_HOST_TO_DEVICE), 0);                       \
        aclshmemi_control_barrier_all();                                                                               \
        test_mte_##NAME##_put_nbi(block_dim, stream, gva, util_get_ffts_config());                                     \
        ASSERT_EQ(aclrtSynchronizeStream(stream), 0);                                                                  \
        aclshmemi_control_barrier_all();                                                                               \
        ASSERT_EQ(aclrtMemcpy(outHost, totalSize, gva, totalSize, ACL_MEMCPY_DEVICE_TO_HOST), 0);                      \
        for (uint32_t i = 0; i < pe_size; i++) {                                                                       \
        ...
        ASSERT_EQ(aclrtMemcpy(gva, totalSize, inHost, totalSize, ACL_MEMCPY_HOST_TO_DEVICE), 0);                       \
        aclshmemi_control_barrier_all();                                                                               \
        test_mte_##NAME##_get_nbi(block_dim, stream, gva, util_get_ffts_config());                                     \
        ASSERT_EQ(aclrtSynchronizeStream(stream), 0);                                                                  \
        aclshmemi_control_barrier_all();                                                                               \
        ASSERT_EQ(aclrtMemcpy(outHost, totalSize, gva, totalSize, ACL_MEMCPY_DEVICE_TO_HOST), 0);                      \
        for (uint32_t i = 0; i < pe_size; i++) {                                                                       \
...
#define DEFINE_PUT_GET_NBI_ENTRY(NAME, TYPE)                                                                     \
    void test_aclshmem_mte_##NAME##_put_get_nbi(int pe_id, int n_pes, uint64_t local_mem_size)                   \
    {                                                                                                            \
        int32_t device_id = pe_id % test_gnpu_num + test_first_npu;                                              \
        aclrtStream stream;                                                                                      \
        test_init(pe_id, n_pes, local_mem_size, &stream);                                                        \
        ASSERT_NE(stream, nullptr);                                                                              \
                                                                                                                 \
        test_mte_##NAME##_put_get_nbi_func(stream, (uint8_t*)g_state.heap_base, pe_id, n_pes);                   \
        std::cout << "[TEST] " #NAME "_put_nbi+" #NAME "_get_nbi (MTE) finished, pe_id: " << pe_id << std::endl; \
        test_finalize(stream, device_id);                                                                        \
    }                                                                                                            \
                                                                                                                 \
    TEST(TestTypedPutGetNbiMTE, TestShmem##NAME##PutGetNbiNbiMTE)                                                \
    {                                                                                                            \
        const int processCount = test_gnpu_num;                                                                  \
        uint64_t local_mem_size = 1024UL * 1024UL * 1024;                                                        \
        test_mutil_task(test_aclshmem_mte_##NAME##_put_get_nbi, local_mem_size, processCount);                   \
    }
...
#define DECLARE_PUT_GET_NBI_TENSOR_WRAPPER(NAME, TYPE)                                                             \
    extern void test_mte_##NAME##_put_nbi_tensor(uint32_t block_dim, void* stream, uint8_t* gva, uint64_t config); \
    extern void test_mte_##NAME##_get_nbi_tensor(uint32_t block_dim, void* stream, uint8_t* gva, uint64_t config)
...
#define DEFINE_PUT_GET_NBI_TENSOR_TEST(NAME, TYPE)                                                     \
    static void test_mte_##NAME##_put_get_nbi_tensor_func(                                             \
    ...
    {                                                                                                  \
        ...
        ASSERT_EQ(aclrtMemcpy(gva, totalSize, inHost, totalSize, ACL_MEMCPY_HOST_TO_DEVICE), 0);       \
        aclshmemi_control_barrier_all();                                                               \
        test_mte_##NAME##_put_nbi_tensor(block_dim, stream, gva, util_get_ffts_config());              \
        ASSERT_EQ(aclrtSynchronizeStream(stream), 0);                                                  \
        aclshmemi_control_barrier_all();                                                               \
        ASSERT_EQ(aclrtMemcpy(outHost, totalSize, gva, totalSize, ACL_MEMCPY_DEVICE_TO_HOST), 0);      \
        for (uint32_t i = 0; i < pe_size; i++) {                                                       \
        ...
        ASSERT_EQ(aclrtMemcpy(gva, totalSize, inHost, totalSize, ACL_MEMCPY_HOST_TO_DEVICE), 0);       \
        aclshmemi_control_barrier_all();                                                               \
        test_mte_##NAME##_get_nbi_tensor(block_dim, stream, gva, util_get_ffts_config());              \
        ASSERT_EQ(aclrtSynchronizeStream(stream), 0);                                                  \
        aclshmemi_control_barrier_all();                                                               \
        ASSERT_EQ(aclrtMemcpy(outHost, totalSize, gva, totalSize, ACL_MEMCPY_DEVICE_TO_HOST), 0);      \
        for (uint32_t i = 0; i < pe_size; i++) {                                                       \
...
#define DEFINE_PUT_GET_NBI_TENSOR_ENTRY(NAME, TYPE)                                                              \
    void test_aclshmem_mte_##NAME##_put_get_nbi_tensor(int pe_id, int n_pes, uint64_t local_mem_size)            \
    {                                                                                                            \
        int32_t device_id = pe_id % test_gnpu_num + test_first_npu;                                              \
        aclrtStream stream;                                                                                      \
        test_init(pe_id, n_pes, local_mem_size, &stream);                                                        \
        ASSERT_NE(stream, nullptr);                                                                              \
                                                                                                                 \
        test_mte_##NAME##_put_get_nbi_tensor_func(stream, (uint8_t*)g_state.heap_base, pe_id, n_pes);            \
        std::cout << "[TEST] " #NAME "_put_nbi_tensor+" #NAME "_get_nbi_tensor (MTE) finished, pe_id: " << pe_id \
                  << std::endl;                                                                                  \
        test_finalize(stream, device_id);                                                                        \
    }                                                                                                            \
                                                                                                                 \
    TEST(TestTypedPutGetNbiTensorMTE, TestShmem##NAME##PutGetNbiTensorMTE)                                       \
    {                                                                                                            \
        const int processCount = test_gnpu_num;                                                                  \
        uint64_t local_mem_size = 1024UL * 1024UL * 1024;                                                        \
        test_mutil_task(test_aclshmem_mte_##NAME##_put_get_nbi_tensor, local_mem_size, processCount);            \
    }
