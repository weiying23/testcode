...
#define PREFETCH_STREAM(addr) __builtin_prefetch((const void*)(addr), 0, 1)
#define PREFETCH_DISTANCE 64  // Prefetch distance in bytes (adjust based on cache line size)
            ...
            for (int tz = RADIUS; tz < NZ - RADIUS; tz += TILE_Z) {
                ...
                for (int x = tx; x < tile_x_end; ++x) {
                    for (int y = ty; y < tile_y_end; ++y) {
                        ...
                        //PREFETCH_WRITE(&output[row_start_idx]);
                        ...
                        for (int z = tz; z < tile_z_end; ++z) {
                            ...
                            //PREFETCH_READ(&input[offset_x + NY * NZ + PREFETCH_DISTANCE/sizeof(float)]);
                            for (int k = 0; k < 2 * RADIUS + 1; ++k) {
                                int read_idx = offset_x + k * NY * NZ;
                                sum += input[read_idx];
                            }
                            
                            // y方向 stencil
                            int offset_y = idx - RADIUS * NZ;
                            //PREFETCH_READ(&input[offset_y + NZ + PREFETCH_DISTANCE/sizeof(float)]);
                            for (int k = 0; k < 2 * RADIUS + 1; ++k) {
                                int read_idx = offset_y + k * NZ;
                                sum += input[read_idx];
                            }
                            
                            // z方向 stencil
                            int offset_z = idx - RADIUS;
                            //PREFETCH_READ(&input[offset_z + PREFETCH_DISTANCE/sizeof(float)]);
                            for (int k = 0; k < 2 * RADIUS + 1; ++k) {
                                int read_idx = offset_z + k;
                                sum += input[read_idx];
                            }
