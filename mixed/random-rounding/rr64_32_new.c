 ...
 * ============================================================================
 *
 * Stochastic Rounding (SR) 的数学定义：
 * 对于实数 x，设 fl(x) 为其浮点表示，SR 以概率 p 向上舍入到 fl⁺(x)，
 * 以概率 1-p 向下舍入到 fl⁻(x)，其中：
 *
 *   p = (x - fl⁻(x)) / (fl⁺(x) - fl⁻(x))
 *
 * 关键性质：E[SR(x)] = x （无偏性）
 *
 * -----------------------------------------------------------------------------
 * 步骤 1: 提取 IEEE 754 二进制表示
 * -----------------------------------------------------------------------------
 * float 在内存中的格式 (binary32):
 *   [31]     [30:23]      [22:0]
 *   符号位 S | 指数 E | 尾数 M
 *
 * 实际值 = (-1)^S × 2^(E-127) × (1.M)  [规格化数]
 *
 * 例如：x = 0.15f 的二进制表示约为
 *   0 01111100 00110011001100110011010
 *   S=0, E=124, M=0x19999A
 *
 * -----------------------------------------------------------------------------
 * 步骤 2: 添加隐含的 1 (implicit leading bit)
 * -----------------------------------------------------------------------------
 * IEEE 754 规格化数的尾数有一个隐含的前导 1：
 *   存储的尾数 = 0x19999A (23 位)
 *   完整的尾数 = 1 00110011001100110011010 (24 位) = 0x19999A | 0x800000
 *
 * 这个隐含位不参与存储，但在计算时必须加上。
 *
 * -----------------------------------------------------------------------------
 * 步骤 3: 计算需要丢弃的比特数 k
 * -----------------------------------------------------------------------------
 * binary32 有 24 位有效数字 (23 存储 + 1 隐含)。
 *
 *   目标精度           precision_bits    需要丢弃的比特数 k
 *   binary16 (半精度)    11              k = 24 - 11 = 13
 *   bfloat16 (脑浮点)     8              k = 24 - 8  = 16
 *
 * k 的含义：我们需要从 24 位尾数中舍入到 precision_bits 位，
 * 即丢弃最低的 k 位。
 *
 * -----------------------------------------------------------------------------
 * 步骤 4: 生成 k 位随机数
 * -----------------------------------------------------------------------------
 * 生成一个 k 位的随机整数 r，范围是 [0, 2^k - 1]。
 *
 * 例如 k = 3 时，random_mask = 0b111 = 7，r ∈ {0,1,2,3,4,5,6,7}
 *
 * 【随机数策略】根据 k 的大小选择最佳方式：
 *   - k <= 15: 使用 (rand() >> (31-k)) & mask 取高位比特
 *   - k > 15:  使用 rand() & mask 取低位比特
 * 原因：LCG 的低位随机性差，但移位过多会导致随机数不足。
 *       bfloat16 (k=16) 需要 16 位随机数，直接用低位更可靠。
 *
 * -----------------------------------------------------------------------------
 * 步骤 5: 尾数 + 随机数，然后截断
 * -----------------------------------------------------------------------------
 * 这是 SR 的高效实现技巧，等价于：
 *
 *   以概率 p = frac(x) / ulp 向上舍入
 *
 * 其中 frac(x) 是 x 的小数部分，ulp 是最低有效位的值。
 *
 * 原理证明：
 * 设尾数为 m，要丢弃低 k 位。令：
 *   m_high = m >> k (保留的高位)
 *   m_low  = m & ((1<<k)-1) (要丢弃的低位)
 *
 * 向上舍入的条件是：m_low + r >= 2^k
 *
 * 由于 r 在 [0, 2^k-1] 均匀分布：
 *   P[向上舍入] = P[r >= 2^k - m_low] = m_low / 2^k
 *
 * 这正是 SR 所需的概率！
 *
 * -----------------------------------------------------------------------------
 * 步骤 6: 处理进位
 * -----------------------------------------------------------------------------
 * 如果 尾数 + 随机数 >= 2^24，说明产生了进位：
 *   - 尾数右移 1 位 (除以 2)
 *   - 指数 +1
 *
 * 例如：24 位尾数全为 1 时，加随机数可能溢出。
 *
 * 如果指数达到 0xFF (255)，结果变为无穷大 (Inf)。
 *
 * -----------------------------------------------------------------------------
 * 步骤 7: 截断低位并重组
 * -----------------------------------------------------------------------------
 * 清零低 k 位：new_mantissa = mantissa_64 & ~((1<<k)-1)
 *
 * 移除隐含的 1 (存储时不需要)，重组符号、指数、尾数：
 *   new_bits = (sign << 31) | (exp << 23) | new_mantissa
 *
 * -----------------------------------------------------------------------------
 * 示例：stochastic_round(0.15f, PRECISION_BFLOAT16)
 * -----------------------------------------------------------------------------
 * 输入：x = 0.15f
 *   二进制：0 01111100 00110011001100110011010
 *   完整尾数 (24 位): 100110011001100110011010
 *
 * bfloat16 只有 8 位尾数，需要丢弃 k = 24-8 = 16 位。
 *
 * 假设随机数 r = 0xABCD (16 位随机值)
 *
 *   完整尾数    100110011001100110011010
 * + 随机数      000000001010101111001101
 * ------------------------------------
 * = 扩展尾数    100110011100010101101011 (25 位)
 *
 * 截断低 16 位：10011001 (8 位，保留的高位)
 *
 * 输出：0 01111100 10011001000000000000000
 *       (bfloat16 格式，低 16 位尾数为 0)
 */
float stochastic_round(float x, int precision_bits) {
    ...
    // 【修复】根据 k 的大小选择最佳随机数生成策略：
    //   - k <= 15: 使用高位比特 (LCG 高位随机性更好)
    //   - k > 15:  使用低位比特 (避免移位后随机数不足)
    uint32_t random_mask = (1U << k) - 1;           // k 位全 1 的掩码
    uint32_t random_bits;
    if (k <= 15) {
        // 小 k 值：取高位比特，避免 LCG 低位周期性问题
        random_bits = (rand() >> (31 - k)) & random_mask;
    } else {
        // 大 k 值 (如 bfloat16 的 k=16)：使用原始 rand() 的低位
        // 因为 rand() 通常有 31 位，低 16 位也是足够随机的
        random_bits = rand() & random_mask;
    }
 ...
 *
 * 【重要】使用 FP64 (double) 累加，最后舍入一次
 *
 * 原因：
 * 1. CG 算法对点积精度非常敏感
 * 2. 每次运算后舍入会导致误差累积过快（n 次 × 2 次舍入/次 = 400 次）
 * 3. bfloat16 只有 8 位精度，SR 的随机误差方差过大
 * 4. 随机误差累积会破坏收敛判据的稳定性，导致无法收敛
 *
 * 【说明】这不是真实的 FP32 低精度模拟
 * - 真实 FP32 硬件点积：使用 FP32 累加器，每次运算后舍入
 * - 本实现：使用 FP64 累加器，仅最后舍入一次
 *
 * 【设计理由】
 * 1. 参考 NVIDIA Tensor Core 设计：
 *    - 输入：FP16/bfloat16
 *    - 内部累加：FP32（更高精度）
 *    - 输出：FP16/bfloat16
 *    本实现采用类似策略：输入 FP32，累加用 FP64，输出 FP32
 *
 * 2. CG 收敛性需求：
 *    - 点积用于计算残差 rho = r'r
 *    - 如果 rho 波动过大，收敛判据 |rho| < tol 无法稳定判断
 *    - 实验表明：每次舍入导致 BF16 SR 无法收敛
 *
 * 3. 误差分析目的：
 *    - 本代码重点研究"存储/通信"时的低精度舍入误差
 *    - 而非"计算过程"中的舍入误差
 *    - 点积代表"全局归约"操作，仅最终结果需要低精度化
 *
 * @return 点积结果（FP32，已按指定精度舍入）
 */
static float dot_product(int n, const float* x, const float* y, int prec_bits, int use_sr) {
    double sum = 0.0;  // 【关键】使用 FP64 累加，而非 FP32
    for (int i = 0; i < n; i++) {
